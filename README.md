...
1. 连接设备后，操作员在当前界面观察波形质量（`quality_check_sec` 仅作建议观察时长，不写入正式事件流）
2. `calibration`（open/closed/eye/blink/swallow/jaw/head）
3. `practice`（支持操作员确认后提前结束）
4. `MI run 1`
5. `MI run 2`
...
- 数据采集：保存原始 EEG，不在采集端对落盘数据做滤波、去趋势或其他预处理
...
采集时序可调建议（用于后续个体化，不改变当前默认）：

- calibration：`eye` 可在 `20-30s` 调整；`blink/swallow/jaw/head` 可在 `15-20s` 调整
- practice：新手建议 `180s`；熟悉受试者可降到 `120s`，并用 `N` 提前结束
- idle：`idle_block` 建议 `2 x 60-90s`；`idle_prepare` 建议至少 `2 x 60s`
- continuous：标准版 `2 x 240s`；若想分布更均匀可改 `3 x 180s`
- continuous 命令/间隔：新手建议 `4-5s` + `2-3s`；熟练后可尝试 `3.5-5s` + `1.5-2.5s`
...
- `N`：提前结束当前 `practice`（或 `quality_check`）阶段，进入下一阶段（仅在会话运行且未暂停时生效）
- `Esc`：停止并保存
...
- 右侧 EEG 预览是“仅显示链路”，用于更稳地观察漂移和工频噪声
- 预览显示链路：`detrend + 1Hz 高通 + 50Hz 陷波 + 40Hz 低通`
- 阻抗模式显示仍为 raw（不走 EEG 显示滤波链路）
- 所有保存文件（`*_raw.fif` / `*_events.csv` / `*_trials.csv` / `*_npz`）都基于采集原始信号与标记，不写入显示滤波结果
...
- 统一默认来源：`code/shared/src/preprocessing.py`（`preprocess()` 及其 `DEFAULT_PREPROCESS_*` 常量）
...
guided 模式默认时序已对齐采集协议：`baseline/cue/imagery/iti = 2.0/1.0/4.0/2.0s`。  
若修改 `code/realtime/mi_realtime_infer_only.py` 中的 `protocol_*`，则该次 realtime 属于“自定义测试协议”。
